...
title(lbl_low, 'FontSize', 13);
legend('D = br^D', 'L = 2\bar{r} - br^L', 'Location', 'best');
grid on;
...
%% ---- Panel 5: percentage deviation in aggregate D and L from base ----
yl_p5 = shared_ylim(D_dev_low, L_dev_low, D_dev_star, L_dev_star);

figure('Position', [100 100 1200 450]);

subplot(1,2,1);
plot(tt, D_dev_low, 'b-',  'LineWidth', 1.5); hold on;
plot(tt, L_dev_low, 'r--', 'LineWidth', 1.5);
ylim(yl_p5);
yline(0, 'k:', 'LineWidth', 1.0);
xlabel('Time', 'FontSize', 13); ylabel('% deviation from base', 'FontSize', 13);
title(lbl_low, 'FontSize', 13);
legend('D_s', 'L_s', 'Location', 'best');
grid on;

subplot(1,2,2);
plot(tt, D_dev_star, 'b-',  'LineWidth', 1.5); hold on;
plot(tt, L_dev_star, 'r--', 'LineWidth', 1.5);
ylim(yl_p5);
yline(0, 'k:', 'LineWidth', 1.0);
xlabel('Time', 'FontSize', 13); ylabel('% deviation from base', 'FontSize', 13);
title(lbl_star, 'FontSize', 13);
legend('D_v', 'L_v', 'Location', 'best');
grid on;

sgtitle('Percentage deviation in aggregate deposits and lending from base', 'FontSize', 14, 'FontWeight', 'bold');
